...
char** splitLine(char* lsline){
	char **args = malloc(sizeof(char*)*50); //the name "malloc" stands for memory allocation.
	char *arg;
...
int launch(char** args){
	...
	char *cmdargs2[]={
      ...
      if(strcmp(args[0],"switch")==0){
		...
		if((process=wait(&status))<0){ //It is undesirable for a parent process to complete and close before its own child process. For this reason, there is a wait (& status) command that can be used by the parent to avoid such situations.
			_exit(1);
	    }
	}
}
